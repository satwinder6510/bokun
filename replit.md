...
The admin panel supports image uploads for flight packages. Images are stored locally in `public/uploads/` directory.

### Current Status

**Local Storage:** Images uploaded via the admin panel are stored locally at `/uploads/`. This works in development but images are **not persisted across deployments**.

### Object Storage for Production

For production deployment persistence, Replit Object Storage (App Storage) is available. The infrastructure code is ready in:
- `server/objectStorage.ts` - Object Storage service using Google Cloud Storage
- `server/objectAcl.ts` - Access control for private objects

**To Enable Object Storage:**
1. Go to the Replit "Object Storage" panel and create a new bucket
2. Set environment variable `PRIVATE_OBJECT_DIR` to your bucket path (e.g., `/your-bucket-name/uploads`)
3. Optionally set `PUBLIC_OBJECT_SEARCH_PATHS` for public assets
4. Update the upload endpoints in `server/routes.ts` to use ObjectStorageService

The upload endpoints currently use local storage but are structured for easy migration to Object Storage.