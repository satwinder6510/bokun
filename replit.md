...
**Multi-Currency Support:** The platform supports 5 currencies (USD, EUR, GBP, CAD, INR) with server-side currency conversion via Bokun API. The search endpoint accepts a `currency` query parameter which is included in the HMAC signature. Per-currency caching is implemented: once a currency is fetched, it's cached for 30 days, providing instant switching between previously viewed currencies. First-time currency selection fetches from the API, subsequent selections use the cache. Currency selection is persisted in local storage and the frontend automatically refetches only when switching to an uncached currency.
