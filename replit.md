...
**Holiday Packages (Custom Offers):** In addition to Bokun API tours, the platform supports manually-added custom holiday packages stored in PostgreSQL. These packages display in a dedicated "Holiday Packages" section on the homepage with featured offers prominently displayed. Each package includes detailed information: destination, duration, meal plan, price (in GBP), overview, what's included, highlights, day-by-day itinerary, accommodation details, and image gallery. Custom offers have their own detail pages at `/offer/:slug` with a booking sidebar and enquiry links. Three sample packages are seeded: Procida Italy (£299), Malaysia/Singapore/Bali (£1499), and Sardinia (£399).
